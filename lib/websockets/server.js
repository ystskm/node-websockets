...
function WebSocketServer(options) {
  ...
  self.on('upgrade', function(req, socket, upgradeHead) {
    ...
    conn.on('broadcast', function(message){
      self.emit('broadcast', message, conn);
    });
  ...
  self.on('broadcast', function(message, conn, site) {
    for( var i in connections)
      for(var j in connections[i])
        connections[i][j].send(message);
  });
}
util.inherits(WebSocketServer, events.EventEmitter);

WebSocketServer.prototype.broadcast = function(message, conn) {
  this.emit('broadcast', message, conn);
};
